...
if [ -n "${SSHFS_HOST+set}" ]; then
  ...
  if [ -n "${SSHFS_USER+set}" ] && [ -n "${SSHFS_DIR+set}" ]; then
    ...
    sshfs ${SSHFS_USER}@${SSHFS_HOST}:${SSHFS_DIR} /home/vcap/misc -o IdentityFile=/home/vcap/app/.profile.d/id_rsa -o StrictHostKeyChecking=yes -o UserKnownHostsFile=/home/vcap/app/.profile.d/known_hosts -o idmap=user -o compression=yes -o cache=yes -o kernel_cache -o large_read -o Ciphers=aes256-ctr -o cache_timeout=115200 -o attr_timeout=115200
  else
