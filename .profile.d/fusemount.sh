...
if [ -n "${SSHFS_HOST+set}" ]; then
  ...
  if [ -n "${SSHFS_NAMESPACE+set}" ]; then
    ...
    if [ -f "/home/vcap/misc/${SSHFS_NAMESPACE}/wp-content/index.php" ]; then
      ...
      tar -C /home/vcap/app/htdocs/mirage -jcf - ./ | ssh -i /home/vcap/app/.profile.d/id_rsa -o UserKnownHostsFile=/home/vcap/app/.profile.d/known_hosts ${SSHFS_USER}@${SSHFS_HOST} "tar -C/home/paramount/${SSHFS_NAMESPACE}/wp-content -xf -"
